...
import { LayoutDashboard, Users, Factory, Shirt, Menu, X, LogOut, Settings, ArrowLeft } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, onNavigate, onLogout, onExit, companySettings }) => {
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50">
      ...
      {!isSidebarOpen && (
        <button className="fixed top-4 left-4 z-30 p-2 bg-sow-green rounded-md text-white md:hidden shadow-lg" onClick={() => setIsSidebarOpen(true)}>
          <Menu size={24} />
        </button>
      )}

      <aside className={`fixed md:relative z-50 h-full w-64 bg-sow-black text-white transition-transform duration-300 ease-in-out flex flex-col shadow-xl md:shadow-none ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-800">
          <div className="flex items-center gap-3">
             {companySettings.logoUrl ? (
                <img src={companySettings.logoUrl} alt="Logo" className="w-8 h-8 object-contain bg-white rounded" />
             ) : (
        ...
        {onExit && (
            <div className="p-4 border-b border-gray-800">
                <button onClick={onExit} className="w-full flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors text-sm font-bold">
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === item.id ? 'bg-sow-green text-white shadow-lg shadow-green-900/20' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
            >
              <item.icon size={20} />
              {item.label}
            </button>
        ...
        <div className="p-6 border-t border-gray-800 bg-sow-black">
           <button onClick={onLogout} className="flex items-center gap-2 text-gray-400 hover:text-red-400 text-sm mb-4 w-full transition-colors">
             <LogOut size={16} /> <span>Sair do Sistema</span>
           </button>
           <p className="text-[10px] text-gray-500 leading-tight">{companySettings.footerText}</p>
        </div>
      </aside>
