...
import { Plus, Factory, Edit2, Trash2, X, Loader2 } from 'lucide-react';
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input required className={INPUT_STYLE} value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ex: Têxtil Silva" />
              </div>
              <div>
                <label className={LABEL_STYLE}>Categoria</label>
                <select className={INPUT_STYLE} value={formData.category || 'Costura'} onChange={e => setFormData({...formData, category: e.target.value as SupplierCategory})}>
                    {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className={LABEL_STYLE}>Nome do Responsável / Contato</label>
                <input required className={INPUT_STYLE} value={formData.contact || ''} onChange={e => setFormData({...formData, contact: e.target.value})} placeholder="Ex: Maria" />
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-4">
                ...
                <button type="submit" disabled={saving} className="px-6 py-2 bg-sow-green text-white font-bold rounded-lg hover:bg-green-600 shadow-lg shadow-green-100 transition-colors flex items-center gap-2">
