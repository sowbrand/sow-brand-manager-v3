            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={productionData}>
                ...
                <Bar dataKey="qtd" fill="#36A2EB" radius={[4, 4, 0, 0]} barSize={50}>
                    {productionData.map((entry, index) => (<Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
             ...
             <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={statusData} cx="50%" cy="50%" innerRadius={80} outerRadius={110} paddingAngle={5} dataKey="value">
                  {statusData.map((entry, index) => (<Cell key={`cell-${index}`} fill={['#72bf03', '#FFCE56', '#FF6384'][index]} />))}
                </Pie>
                <Tooltip />
